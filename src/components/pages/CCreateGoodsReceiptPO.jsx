...
const CCreateGoodsReceiptPO = ({ initialData, messages }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const fetchCurrentGoodsReceiptPO = async () => {
                    if (query?.docEntry && query?.base) {
                        ...
                        const res = await fetch(`/api/purchase/get-goods-receipt-po?id=${docEntry}`, {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                    ...
                    // const res = await fetch(`/api/purchase/get-goods-receipt-po/`, {
                    //     method: 'GET',
                    //     headers: {
                    //         'Content-Type': 'application/json',
                    //     },
    ...
    const handleChangeSelectedSupplier = (cardCode) => {
        ...
        setGeneralInfo(prev => ({
            ...prev,
            JournalRemark: 'Goods Receipt PO - ' + selectedValue.CardCode,
    ...
    const handleAddAndView = async () => {
        ...
        try {
            ...
            if (data.DocEntry) {
                router.push("/purchasing/goods-receipt-po/" + data.DocEntry)
            }
    ...
    return (
        <>
            <Head>
                <title>{capitalizeWords(`${tG('create')} ${tG('goodsReceiptPO')}`)}</title>
                                    ...
                                    <div className="card p-2 sm:p-4 relative">
                                        ...
                                        <section>
                                            ...
                                            <div className="grid gap-4 grid-cols-1 sm:grid-cols-3 md:grid-cols-4 p-[7px]">
                                                <div className="flex flex-column gap-2">
                                                    ...
                                                    <div className="flex">
                                                        <Dropdown checkmark filter options={dataLoaded ? supplierListOptions : []} value={supplierListOptions?.find(val => val.code == selectedSupplier?.CardCode)}
                                                            ...
                                                            optionLabel="name" placeholder={t('selectSupplier')} onChange={(e) => handleChangeSelectedSupplier(e.value.code)} className="w-full" />
                                                        <Button icon="pi pi-list" onClick={() => { setSupplierSelectModalOpen(true) }} />
                                                    </div>
                                                </div>
                                                <div className="flex flex-column gap-2">
                                                    <label className='font-semibold'>{t('supplierName')}</label>
                                                    <div className="w-full p-inputtext p-variant-filled p-component flex-1 !min-h-[42px]"><span className="h-[21px]">{selectedSupplier?.CardName || ""}</span></div>
                                                </div>
                                                <div className="flex flex-column gap-2">
                                                ...
                                                <div className="flex flex-column gap-2">
                                                    ...
                                                    <InputText value={generalInfo.NumAtCard} onChange={() => setGeneralInfo(prev => ({ ...prev, NumAtCard: e.target.value }))} variant="filled" aria-describedby="supplier-ref-no" />
                                                </div>
                                                {/* <div className="flex flex-column gap-2">
                                        ...
                                        <section>
                                            ...
                                            <div className="mt-4">
                                                {dataLoaded ?
                                                    (
                                                        <BlockUI autoZIndex={false} baseZIndex={1000} blocked={!selectedSupplier} template={<i className="pi pi-lock" style={{ fontSize: '3rem' }}></i>}>
                                                            <DataTable
...
export default withAuth(CCreateGoodsReceiptPO);
